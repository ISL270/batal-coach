...
final class SettingsIsarSource extends IsarSource<SettingsState, SettingsIsar> {
  ...
  @override
  SettingsIsar fromDomain(SettingsState domain) => SettingsIsar.fromDomain(domain);
  ...
  Future<void> updateLanguage(Language newLanguage) async {
    ...
    if (currentState != null) {
      ...
      await putSettings(SettingsState(const Initial(), newLanguage, ThemeMode.system));
    }
  }
  ...
  Future<void> toggleLanguage() async {
    ...
    if (currentState != null) {
      final newLanguage =
          currentState.language == Language.arabic ? Language.english : Language.arabic;
      final updatedState = currentState.copyWith(language: newLanguage);
      await putSettings(updatedState);
    } else {
      // Initialize with default state if currentState is null
      await putSettings(const SettingsState(Initial(), Language.english, ThemeMode.system));
    }
  }
  ...
  Future<void> updateTheme(ThemeMode newThemeMode) async {
    ...
    if (currentState != null) {
      ...
      // Initialize with default state if currentState is null
      await putSettings(SettingsState(const Initial(), Language.english, newThemeMode));
    }
  }
}
